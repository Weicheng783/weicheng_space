<?php
    ...
    if($_REQUEST['checkedobj'] == null | $_REQUEST['checkedobj'] == "" | $_REQUEST['checkedobj'] == "CONFIRM"){echo "<script>location.href='index.php'</script>";}
    else{
        ...
        for($x=0; $x<count($del_array); $x++){
            $sql = "DELETE FROM `events` WHERE `events`.`id` = $del_array[$x]";
        ...
        if($re){
            ...
            // 这里结束
            $client4 = new AipSpeech(APP_ID, API_KEY, SECRET_KEY);

            if($re){
                $resulteddel2 = $client4->synthesis('所选项已删除成功！', 'zh', 1, array(
                    'vol' => 5,
                    'per' => 3,
                ));
                ...
                if(!is_array($resulteddel2)){
                    file_put_contents('audio.mp3', $resulteddel2);
